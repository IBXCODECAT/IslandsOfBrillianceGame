...
namespace EasyBuildSystem.Runtimes.Internal.Part
{
    ...
    public class PartBehaviour : MonoBehaviour
    {
        ...
        {
            if (BuildManager.Instance.BuildingSupport != SupportType.TerrainCollider || BuildManager.Instance.BuildingSupport != SupportType.VoxelandCollider)
                return false;
            ...
            {
#if VOXELAND
                if (Colliders[i].GetComponentInParent<VoxelandCollider>())
                    return true;
#else
                if (Colliders[i] as TerrainCollider)
                    return true;
#endif
            }
                ...
                if (Type == PartType.Foundation)
                {
                        ...
                        UseGroundUpper ? GroundUpperHeight : 0.3f, Physics.AllLayers))
                    {
                        ...
                        else
                        {
                            ...
                            #else
                            if (BuildManager.Instance.BuildingSupport == SupportType.SurfaceCollider)
                            {
        ...
        public bool CheckElementsClipping()
        {
            Collider[] Colliders = PhysicExtension.GetNeighborsTypesByBox<Collider>(GetWorldPartMeshBounds().center,
                GetWorldPartMeshBounds().extents, transform.rotation, BuildManager.Instance.FreeLayers).Where(x => !x.isTrigger).ToArray();
