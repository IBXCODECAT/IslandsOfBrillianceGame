...
namespace EasyBuildSystem.Runtimes.Internal.Storage
{
    ...
    public class BuildStorage : MonoBehaviour
    {
        ...
        public void LoadInEditor(string path)
        {
            ...
            for (int i = 0; i < Serializer.Prefabs.Count; i++)
            {
                if (Serializer.Prefabs[i] != null)
                {
                    PartBehaviour Prefab = Manager.GetPart(Serializer.Prefabs[i].Id);

                    if (Prefab != null)
                    {
                        PartBehaviour PlacedPrefab = Manager.PlacePrefab(Prefab,
                            PartModel.ParseToVector3(Serializer.Prefabs[i].Position),
                            PartModel.ParseToVector3(Serializer.Prefabs[i].Rotation),
                            PartModel.ParseToVector3(Serializer.Prefabs[i].Scale),
                            Group, null);

                        PlacedPrefab.name = Prefab.Name;
                        PlacedPrefab.transform.position = PartModel.ParseToVector3(Serializer.Prefabs[i].Position);
                        PlacedPrefab.transform.eulerAngles = PartModel.ParseToVector3(Serializer.Prefabs[i].Rotation);
                        PlacedPrefab.transform.localScale = PartModel.ParseToVector3(Serializer.Prefabs[i].Scale);
                        ...
                        Debug.Log("<b><color=cyan>[Easy Build System]</color></b> : The Prefab (" + Serializer.Prefabs[i].Id + ") does not exists in the Build Manager.");
                }
            }
        ...
        private IEnumerator LoadDataFile()
        {
            ...
            if (ExistsStorageFile() || StorageType == StorageType.Android)
            {
                ...
                for (int i = 0; i < Serializer.Prefabs.Count; i++)
                {
                    if (Serializer.Prefabs[i] != null)
                    {
                        PartBehaviour Prefab = BuildManager.Instance.GetPart(Serializer.Prefabs[i].Id);
                        ...
                        if (Prefab != null)
                        {
                            ...
                            PlacedPrefab.name = Serializer.Prefabs[i].Name;
                            PlacedPrefab.transform.position = PartModel.ParseToVector3(Serializer.Prefabs[i].Position);
                            PlacedPrefab.transform.eulerAngles = PartModel.ParseToVector3(Serializer.Prefabs[i].Rotation);
                            PlacedPrefab.transform.localScale = PartModel.ParseToVector3(Serializer.Prefabs[i].Scale);
                            PlacedPrefab.ChangeAppearance(Serializer.Prefabs[i].AppearanceIndex);
                            PlacedPrefab.ExtraProperties = Serializer.Prefabs[i].Properties;
                            ...
                            Debug.Log("<b><color=cyan>[Easy Build System]</color></b> : The prefab (" + Serializer.Prefabs[i].Id + ") does not exists in the list of Build Manager.");
                    }
                }
                ...
                if (!LoadAndWaitEndFrame)
                    ...
                    Debug.Log("<b><color=cyan>[Easy Build System]</color></b> : Data file loaded " + PrefabLoaded + " prefab(s).");
        ...
        private IEnumerator SaveDataFile()
        {
            ...
            if (BuildManager.Instance.Parts.Count > 0)
            {
                ...
                PartBehaviour[] PartsAtSave = BuildManager.Instance.Parts.ToArray();
                    ...
                    if (PartsAtSave[i] != null)
                    {
                        if (PartsAtSave[i].CurrentState == StateType.Placed || PartsAtSave[i].CurrentState == StateType.Remove)
                        {
                            PartModel.SerializedPart DataTemp = new PartModel.SerializedPart
                            {
                                ...
                                Position = PartModel.ParseToSerializedVector3(PartsAtSave[i].transform.position),
                                Rotation = PartModel.ParseToSerializedVector3(PartsAtSave[i].transform.eulerAngles),
                                Scale = PartModel.ParseToSerializedVector3(PartsAtSave[i].transform.localScale),
