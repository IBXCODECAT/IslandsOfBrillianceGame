    ...
    public class SocketBehaviour : MonoBehaviour
    {
        ...
        public void DisableCollider()
        {
            ...
            if (CachedCollider != null)
                CachedCollider.gameObject.layer = LayerMask.NameToLayer(Constants.LAYER_IGNORE);
        }
        ...
        /// </summary>
        readonly Collider[] NonAllocColliders = new Collider[PhysicExtension.MAX_ALLOC_COUNT];
        public OccupancyType CheckState(LayerMask freeLayers, PartBehaviour placedPart)
        {
            int ColliderCount = Physics.OverlapSphereNonAlloc(transform.position, Radius, NonAllocColliders, freeLayers, QueryTriggerInteraction.Ignore);

            for (int i = 0; i < ColliderCount; i++)
            {
                if (AttachedPart != placedPart)
                {
                    if (NonAllocColliders[i] as TerrainCollider == null)
                        return OccupancyType.Busy;
                }
                else
                {
                    if (!AttachedPart.HasCollider(NonAllocColliders[i]) && NonAllocColliders[i] as TerrainCollider == null)
                        if (NonAllocColliders[i].GetComponent<SocketBehaviour>() != null || NonAllocColliders[i].GetComponentInParent<PartBehaviour>() != null)
                            return OccupancyType.Busy;
                }
            }
        ...
        public void ChangeAreaState(OccupancyType state, LayerMask freeLayer, LayerMask socketLayer, PartBehaviour placedPart)
        {
            ...
            int ColliderCount = Physics.OverlapBoxNonAlloc(placedPart.GetWorldPartMeshBounds().center,
                placedPart.GetWorldPartMeshBounds().extents, NonAllocColliders,
                placedPart.transform.rotation, socketLayer, QueryTriggerInteraction.Collide);

            for (int i = 0; i < ColliderCount; i++)
            {
                SocketBehaviour Socket = NonAllocColliders[i].GetComponent<SocketBehaviour>();
            ...
            ColliderCount = Physics.OverlapSphereNonAlloc(transform.position, Radius, NonAllocColliders, freeLayer);

            for (int i = 0; i < ColliderCount; i++)
            {
                if (NonAllocColliders[i].GetComponent<SocketBehaviour>() == null)
                {
                    PartBehaviour Part = NonAllocColliders[i].GetComponentInParent<PartBehaviour>();
