            ...
            try
            {
                ...
                if (NavigationIndex == 0)
                {
                    ...
                    EditorGUILayout.HelpBox("The add-ons can will be only activated directly on their target component in add-ons section.\n" +
                        "If you want to share your add-on here, feel free to send us a request.", MessageType.Info);
                    ...
                    else
                    {
                        foreach (AddOnAttribute Addon in Addons)
                        {
                            ...
                            GUILayout.Label("Author : " + Addon.Author, EditorStyles.miniLabel);
                            GUILayout.Label("Target : " + Addon.Target.ToString(), EditorStyles.miniLabel);

                            GUILayout.BeginVertical();
                            GUILayout.Label("Description : " + Addon.Description, EditorStyles.miniLabel);
