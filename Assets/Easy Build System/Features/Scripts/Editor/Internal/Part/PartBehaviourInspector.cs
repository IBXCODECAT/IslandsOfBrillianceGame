    ...
    public class PartBehaviourInspector : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (BasePartFoldout)
            {
                ...
                EditorGUILayout.PropertyField(serializedObject.FindProperty("FreePlacement"), new GUIContent("Part Free Placement :", "This allows to place your part in free placement."));

                EditorGUILayout.PropertyField(serializedObject.FindProperty("AvoidClipping"), new GUIContent("Part Avoid Clipping :", "This allows to prevents the preview from entering other objects placed in the scene."));

                EditorGUILayout.PropertyField(serializedObject.FindProperty("AvoidClippingOnSocket"), new GUIContent("Part Avoid Clipping On Socket :", "This allows to prevents the preview from entering other objects placed in the scene (Only on socket)."));

                EditorGUILayout.PropertyField(serializedObject.FindProperty("RequireSockets"), new GUIContent("Part Placeable Only On Sockets :", "This allows to define if this part has need a socket for be placed."));
                
                EditorGUILayout.PropertyField(serializedObject.FindProperty("RequireAreas"), new GUIContent("Part Placeable Only On Areas :", "This allows to define if this part has need a area for be placed (require:Area Behaviour allow also this part)."));

                EditorGUILayout.PropertyField(serializedObject.FindProperty("AvoidAnchoredOnSocket"), new GUIContent("Part Avoid Snapped On Sockets :", "This allows to avoid the clipping on the socket(s) which contains this part in offsets list."));

                EditorGUILayout.PropertyField(serializedObject.FindProperty("AvoidDestruction"), new GUIContent("Part Avoid Destruction :", "This allows to avoid the destruction of the part during the destruction mode."));
            }
            ...
            if (PreviewPartFoldout)
            {
                ...
                if (serializedObject.FindProperty("UseGroundUpper").boolValue)
                    EditorGUILayout.PropertyField(serializedObject.FindProperty("GroundUpperHeight"), new GUIContent("Preview Ground Upper Height :", "This allows to define the maximum limit not to exceed on the Y axis."));

                EditorGUILayout.PropertyField(serializedObject.FindProperty("RotateOnSockets"), new GUIContent("Preview Rotate On Socket(s) :", "This allows to define if the preview can be rotated on socket."));
            ...
            if (serializedObject.FindProperty("AdvancedFeatures").boolValue)
            {
                ...
                if (AppearanceFoldout)
                {
                    ...
                    if (Target.Appearances.Count == 0)
                    {
                        ...
                        GUILayout.Label("The list does not contains of appearances child.");
                ...
                if (PhysicsFoldout)
                {
                    ...
                    {
                        EditorGUILayout.PropertyField(serializedObject.FindProperty("PhysicsOnlyStablePlacement"), new GUIContent("Physics Placeable Only On Stable :", "This allows to allows the placement only if the placement surface is stable."));
                        ...
                        EditorGUILayout.PropertyField(serializedObject.FindProperty("PhysicsConvexOnAffected"), new GUIContent("Physics Support Convex Collider(s) :", "This allows to convex all the mesh colliders in children when the part is affected by the physics."));

                        EditorGUILayout.PropertyField(serializedObject.FindProperty("PhysicsLifeTime"), new GUIContent("Physics Life Time :", "This allows to define the destroy time after that the part has been affected by physics."));
